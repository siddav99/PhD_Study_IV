...
Multivariable linear models was used to study the association of age and gender with the EQ-5D index score and multivariable logistic regression was used to measure the association with the different domains of EQ-5D which have multilevel responses. We estimated 95% confidence intervals and denote associations with a p-value of less than 0.05 as statistically significant [@Carey2013]. The statistical software R was used for all statistical analyses [@RCoreTeam2015].

Study Size
----------

Previous studies indicate that as many as 25 events per variable is adequate to obtain stable estimates for linear<!-- Logistic?--> regression [@Courvoisier2011; @Austin2015]. Given that our model includes around 18 free parameters, we need a minimum sample size of around 450 for the EQ-5D index score<!-- You need to define what you mean by an event here, consider giving an example-->.  For multivariable<!-- Multinomial?--> logistic regression the events per variable is ratio of the smallest number of observations in the multinomial outcome categories divided by the number of free parameters and should be at least 10 [@DeJong2019]. With 18 free parameters, requiring at least 180 events per category of the EQ5D domains, if there are inadequate samples, the categories would be combined as "Having no problems" and "Having any problems"<!-- This sentence is too complicated for me. Do you mean that you did not have a big enough sample size to fulfil the criterion of a minimum of 10 EPVm? If so I suggest you start by explaining the sample size requirement of the multinomial logistic regression, then say that your sample size was not enough, and that you therefore dichotmoized your outcomes and used logistic regression sample size guidelines to estimate your sample size. You also need to say something about the sample size requirements of linear regression, see [@Austin2015] -->. 
...
Our logistic regression analysis in the mobility dimension keeping young males as the reference group, shows that the adjusted odds of reporting mobility problems was `r m.young.fem` in young females (aged 18-32), `r m.middle.fem` middle-aged females (aged 33-59), and `r m.old.fem` old females (aged 60 and above). In middle-aged males (aged 33-49) the odds were `r m.middle.mal` and in old males (aged 50 and above) the odds were `r m.old.mal`. Thus, all groups had higher odds of reporting some mobility problems than young males except middle-aged females. 
