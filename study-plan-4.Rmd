...
Multivariate linear regression was used to study the association of age and gender with the EQ-5D VAS score and multivariate ordinal logistic regression was used to measure the association with the different domains of EQ-5D which have multilevel responses. In order to account for the effects of multicollinearity, variance inflation factor (VIF) would be calculated and those variables having a value above 5 would be standardized by centering (standardized by subtracting the mean). The statistical software R was used for all statistical analyses We estimated 95% confidence intervals and denote associations with a p-value of less than 0.05 as statistically significant [@Carey2013]. The statistical software R was used for all statistical analyses [@RCoreTeam2015].

Study Size
----------

Previous studies indicate that as many as 25 events or subjects per variable is adequate to obtain stable estimates for logistic regression in multivariate analysis [@Courvoisier2011; @Austin2015]. In this study would be a participant in each variable. For example, at least 25 subjects  having railway injuries as mechanism of injury or at least 25 subjects reporting systolic blood pressure. Given that our model includes around 18 free parameters (Table 1), we need a minimum sample size of around 450. Additionally, for multinomial logistic regression the subjects per variable is ratio of the smallest number of observations in the multinomial outcome categories divided by the number of free parameters and should be at least 10 [@DeJong2019]. With 18 free parameters, requiring at least 180 events per category. For example the EQ5D domains of mobility has three outcomes:"No problems in walking about", "Some problems in walking about", "Confined to bed". Therefore there should be at least 180 subjects in each of the three categories. In case of there are inadequate subjects in these three outcomes categories, the categories would be combined into two "Having no problems" and "Having any problems". For linear regression there should be at least 2 subjects per variable [@Austin2015]. Therefore, the EQ 5D VAS score analysis would require a minimum sample of 36.
