...
Statistical Methods
-------------------
<!-- The subgroup analysis is not mentioned here -->
We used multivariable linear regression to study the association of age and gender with the EQ-5D VAS score and multivariable ordinal logistic regression was used to measure the association with the different domains of EQ-5D which have multilevel responses. In order to account for the effects of multicollinearity, we calculated the variance inflation factor (VIF)<!-- Use active voice--> and those variables having a value above 5 were centered by subtracting the mean. The statistical software R was used for all statistical analyses We estimated 95% confidence intervals and denote associations with a p-value of less than 0.05 as statistically significant [@Carey2013]. The statistical software R was used for all statistical analyses [@RCoreTeam2015].<!-- How about standard logisitc regression. That's what you used for the dichotomized outcomes, right? -->

Study Size
----------

Previous studies indicate that as many as 25 events or subjects per variable is adequate to obtain stable estimates for logistic regression in multivariate analysis [@Courvoisier2011; @Austin2015]. In this study would be a participant in each variable. For example, at least 25 subjects  having railway injuries as mechanism of injury or at least 25 subjects reporting systolic blood pressure<!-- Events and subjects are not the same thing. An event is a subject with the outcome of interest. If the outcome is death, an event is a subject who died. You dichotomize you outcomes, so for exmple the EQ5D domain mobility can be conceptulised as a binary variable called "Any mobility problems" with the levels Yes (1) or No (0). An event would then be a subject with Any mobility problems. If you need 25 eents per variable and you have 18 variables (parameters to be estimated to be exact) then you need 450 subject who report any mobility problem and at least as many who report no mobility problems. The total sample size required is therefore a minimum of 900 subjects (450 events + 450 non-events). Is the difference between subjects and events clearer now? -->  . Given that our model includes around 18 free parameters (Table 1), we need a minimum sample size of around 450. Additionally, for multinomial logistic regression the subjects<!-- This should be events, right?--> per variable is ratio of the smallest number of observations in the multinomial outcome categories divided by the number of free parameters and should be at least 10 [@DeJong2019]. With 18 free parameters, requiring at least 180 events per category. For example the EQ5D domains of mobility has three outcomes:"No problems in walking about", "Some problems in walking about", "Confined to bed". Therefore there should be at least 180 subjects in each of the three categories. In case of there are inadequate subjects in these three outcomes categories, the categories would be combined into two "Having no problems" and "Having any problems". For linear regression there should be at least 2 subjects per variable [@Austin2015]. Therefore, the EQ 5D VAS score analysis would require a minimum sample of 36.

<!-- This sentence is too complicated for me. Do you mean that you did not have a big enough sample size to fulfil the criterion of a minimum of 10 EPVm? If so I suggest you start by explaining the sample size requirement of the multinomial logistic regression, then say that your sample size was not enough, and that you therefore dichotmoized your outcomes and used logistic regression sample size guidelines to estimate your sample size. You also need to say something about the sample size requirements of linear regression, see [@Austin2015] -->. 
...
Each center was assigned a center identification number and each patient a locally unique study identification number. Project officers first entered data on paper.<!-- The paper form includes information such as ward number, hospital record number, and phone numbers, so maybe that can be considered personal data?. SD: Dropped it.--> . The project officers then transferred this data to an electronic format using a dedicated data entry application. The electronic data did not include any direct identifiers such as name, hospital record number, and telephone numbers. The only way to link an electronic record to a paper intake form was by combining the record's hospital and study identification numbers. Paper forms were kept locally at each center for the duration required by locally applicable laws and regulations, or at least five years, whichever is longest. The adequacy of their storage was the responsibility of the principal investigator at each center. Care was taken taken to ensure that at no time where they stored with less than reasonable care.
...
Out of a total of `r total.patients` trauma patients enrolled, `r missing.patients` were excluded for missing data. The final cohort of `r final.patients` included in this study had a median age of `r data.description$age.median` years (IQR: `r data.description$age.iqr.upper`-`r data.description$age.iqr.lower`) with `r data.description$proportion.male` per cent of patients were male. The most common mechanisms of injury were  `r data.description$moi1` (`r data.description$p.road` per cent). Majority of the injuries were blunt (`r data.description$proportion.blunt` per cent) and `r data.description$proportion.transferred` per cent were transferred to the study hospitals. The all cause 30-day mortality was `r  data.description$p.outcome.dead` per cent. The median injury severity score was x. Details of the study population is given in Table 2.

EQ-5D Scores
------------
`r hs.complete` completed the EQ-5D questionnaire at 3 months after the trauma. The mean EQ5D health status score was `r hs.mean` (SD = `r hs.sd`). Just over half the patients (54.8%) reported no problems with mobility while two-thirds reported no problems with self-care. While less than half the patients (43.0%) could carry on usual activities without any problems and only one-third of the patients (34.7%)<!-- So, these I took from the Table created using the create_sample_characteristics_table function. Is there a way to extract data from them so that I can create inline codes?--> reported no pain or discomfort after three-months of post-discharge. Around 40% of the patients reported experiencing some form of anxiety or depression. Again, the proportion of young males reporting any problems across all the five domains was the lowest while the proportion  middle-aged females reported experiencing the problems across all the five domains was the highest. The overall EQ-5D scores are provided in Table 3.

EQ-5D in relation to gender and age
------------------------------------

Overall females had a slightly higher mean EQ5D health status score (`r eq5dhs.fem`) than males (`r eq5dhs.mal`). Young females (18-32 years) had the highest health status score (`r eq5dhs.young.fem`) followed by younger males (`r eq5dhs.young.mal`) while middle aged males (`r eq5dhs.middle.mal`) and older females reported the lowest scores (`r eq5dhs.old.fem`). After adjusting for injury etiology, vitals, and severity, this pattern remained the same (Table 3). The linear regression of health status with the interaction of age and gender and after adjusting for injury etiology, vitals, and severity, was not significant<!-- What part of the linear regression was not significant? The interaction effect?-->.


<!-- Basically you're reporting the same thing but for different outcomes below, right? I suggest you start by saying that you conducted logistic regression for each of the outcomes and then list the outcomes. You then report the findings of those analyses in a standardized way, i.e. use the same wording for each outcome. I know that it mat appear repetitive, but I find the current structure hard to follow. Also, some tables would be good here, reporting the results of the regression models-->
Logistic regression analysis in the mobility dimension keeping young males as the reference group, shows that the adjusted odds of reporting mobility problems was `r m.young.fem` in young females (aged 18-32), `r m.middle.fem` middle-aged females (aged 33-59), and `r m.old.fem` old females (aged 60 and above). In middle-aged males (aged 33-49) the odds were `r m.middle.mal` and in old males (aged 60 and above) the odds were `r m.old.mal`. Thus, all groups had lower odds of reporting some mobility problems than young males except middle-aged females. 

Assessing the logistic regression analysis in the self-care dimension, showed the adjusted odds of reporting problems performing self-care tasks was `r sc.young.fem` in young females, `r sc.middle.fem` in middle-aged females, and `r sc.old.fem` old females compared to young males. On the other hand, the odds of reporting these problems in middle-aged males was `r sc.middle.mal` and in old males it was `r sc.old.mal`times than young males. Only middle aged and older females had higher odds of reporting problems with self-care than young males.

In the dimension of being able to perform usual activities, logistic regression analysis indicated that the adjusted odds of reporting any problems in performing was `r ua.young.fem` in young females, `r ua.middle.fem` in middle aged females, and `r ua.old.fem` old females with respect to young males. The odds of reporting these problems were `r ua.middle.mal` in middle aged males and  `r ua.old.mal` times in in old males. Thus, middle aged females had higher odds of having problems performing usual activities than young males.
...
Discussion
==========
This study examined the interaction of age and gender on QOL in trauma patients discharged from four tertiary-care hospital in urban India at 3-months after discharge. We did not find statistically significant differences across all age groups and gender as well as no interaction effect between age and gender in this population. The sub-group analysis showed differences in QOL between males and females across young (18-32 years), middle-aged (33-59), and older (60 years and above) groups. In each of the five EQ5D domains, odds of reporting problems varied between the groups. We found middle-aged males report lower overall QOL scores while middle aged females had the highest odds of reporting problems across almost all the QOL domains.

The mean EQ5D index score of 81.7 of trauma patients in this study is lower than the general population norm for South Asia of 86 [@Kularatna2014]. A recent study of trauma patients from in north India, reported index scores of around 90 at 3-months post-injury, lower<!-- Higher?--> than our findings <!-- Was the case mix similar in their study-->[@Jagnoor2020]. This could be because two of the three study sites of that study were secondary-care hospitals, which treat less severe trauma patients. The score is however, higher than other studies which report scores between  48 to 66 for trauma patients at 3-months<!-- Where are these studies from? What's the case mix like?--> [Spronk2019; @Wihlke2021]. This indicates that, even after 3-months since discharge, trauma patients still face limitations to achieve full recovery for a long-time after trauma. Pain and inability to perform usual activities were the main drivers of poor QOL in this<!-- Our? --> study, with nearly two-thirds of participants reporting some form of pain at 3-months. This is consistent with other literature on similar cohorts in other settings [Spronk2019; @Wihlke2021]. 

In this study females reported slightly better overall QOL than males, which is consistent with the study from North India [@Jagnoor2020]. Evidence on this remains mixed with some studies reporting lower scores for females [@Rissanen2020] while other report lower scores for males [@Jagnoor2020]. Studies that report females outperforming<!-- Outperforming feels weird in this context, other word?--> males also tend to have a smaller proportion of females in their study sample. In this study, the middle-aged cohort, had the highest odds of poor QOL outcomes. The middle-aged males had the lowest EQ-5D index scores than the other groups, followed by middle-aged females (coefficients respectively of −4.86 (95% CI −9.07; −0.65) and −2.01 (95% CI -11.41; 7.37)). Better QOL was reported after 3-months following the injury in younger age adults as well as those over 60 years. This is in contrast with findings from high-income settings where older adults perform worse [@Gabbe2017; @Wihlke2021] but similar to the findings from India [@Jagnoor2020]. Better scores among the younger age groups older<!-- Missing a word?--> could be due to physiological factors and among the old it could be associated with the cultural context of care by informal caregivers leading to overall better health [@Ghosh2020]. 

In this study more females reported pain and barriers to selfcare than their male counterparts across all age groups. On the other hand problems with mobility and usual activities increased with age. These findings are in line with other studies from high-income settings showing that females report more problems in the pain, and usual activities domain than males [@Gabbe2017]. Anxiety and depression also increased with age but after categorizing by gender it was seen that young females reported higher odds than males while older males reported higher odds than females. There is evidence of younger age and female gender being predictive of anxiety and depression after injury in high income settings [@Gabbe2017]. Women having higher prevalence of anxiety and depression has also been observed in the general population in India [@Sagar2020]. The reasons for injured middle-age and older males reporting higher odds for anxiety and depression is unclear. The reasons for this different patterns in reporting anxiety and depression needs to be studied in-depth<!-- Maybe you can merge this and the previous sentence? They seem very similar.-->. However, the proportion of patients reporting anxiety and depression (41%) was almost twice as high as seen in previous studies [@Wihlke2019; @Jagnoor2020]. This underscores the need to account for psychological issues faced by trauma as part of the long-term consequences of trauma in this setting.

Our study showed that while middle-aged females had the highest odds of reporting problems in the mobility, self-care, pain, and usual activities than other groups, they did not report lower overall health status. This could be because of the gendered norms influencing healthcare-seeking behavior among women in the Indian settings, where women may not consider their health status as requiring care despite having more health problems [Agrawal2014; @NationalSampleSurveyOffice2019; @Das2018]. This difference between overall health status and among<!-- Something is missing here--> 

The findings from our study indicate that males and females do not recover QOL at 3-months post trauma the same way across different age groups in the urban Indian context. The reasons for these differences should be explored in detail in future qualitative studies. Additionally, future research should focus on specific cohorts of trauma such as traumatic brain injury or spinal cord injuries and road traffic injuries or falls to study the differences in QOL outcomes and the reasons for the same. The findings of this study highlights that healthcare providers should consider options for addressing the persistent pain among post-discharge trauma patients. Moreover, it  emphasizes the need for  providing psychosocial care as part of trauma management to improve overall QOL among trauma patients. Additionally, given the differences between reporting overall health status and health-related problems faced, especially among women, healthcare providers could probe for these problems during post-discharge follow-ups visits.   

Limitations
-----------
The EQ5D tool has not been validated for trauma patients in the Indian context and conducting a telephonic interview could have affected the ability of participants to understand the questions and thereby the reported responses. However, the tool has been validated in non-trauma settings and culturally similar populations in South Asia. Additionally, the tool has details on how to collect the information over the telephone. This study only captures those who responded during the follow-up calls, leaving out drop-outs. Another limitation is using cross-sectional data that does not allow in-depth analysis of causal relationships of the associations detected in this study<!-- I'm not convinced about the cross-sectional design. Why do you call it cross-sectional rather than cohort? And what do you mean by causal relationship here? -->. Longitudinal studies in the future would helping further determining the causal association between age and gender with QOL in low resource settings.
